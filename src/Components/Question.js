...
function Question(props) {
  const answerButtonsElem = props.allAnswers.map((answer, index) => {
    //answer properties => value, id, isHeld, isCorrect, showAnswer
    //Held Styles
    let styles = {
  ...
  return (
    <div className="question__component">
      <h3 className="question__question">{he.decode(props.question)}</h3>
      <div className="question__btnctr">{answerButtonsElem}</div>
    </div>
  );
}
